...
interface PrintableReturnInvoiceProps {
  data: any; // Menggunakan type dari ReturnInvoice
  onClose: () => void;
}

const PrintableReturnInvoice: React.FC<PrintableReturnInvoiceProps> = ({ 
  data, 
  onClose 
}) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Retur_${data.returnNumber}`,
    onBeforePrint: () => {
      toast({
    ...
    onAfterPrint: () => {
      toast({
        title: "Formulir retur dicetak",
        description: "Dokumen formulir retur telah berhasil dicetak",
      });
    },
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`Retur_${data.returnNumber}.pdf`);
      
      toast({
        title: "PDF berhasil diunduh",
        description: `Dokumen formulir retur ${data.returnNumber} telah berhasil diunduh`,
      });
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center overflow-auto p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-gradient-to-r from-orange-50 to-white">
          <h2 className="text-xl font-semibold text-gray-800">Formulir Retur - {data.returnNumber}</h2>
        ...
        <div className="overflow-auto flex-1 p-4 bg-gray-100">
          <div className="shadow-md print:shadow-none print:m-0">
            <ReturnInvoice ref={invoiceRef} data={data} />
          </div>
        </div>
      </div>
    </div>
...
export default PrintableReturnInvoice;
